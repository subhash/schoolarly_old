...
class ConversationsControllerTest < ActionController::TestCase
  ...
  test "delete conversation should move the entire conversation for the user to trash" do
    assert_difference('@mail.user.mailbox[:trash].mail(:conversation => @conversation).size', 1) do      
  ...
  test "delete conversation should delete the entire conversation for the user" do
    assert_difference('@trashed_mail.user.mail(:conversation => @conversation).size', -1 * @trashed_mail.user.reload.mail(:conversation => @conversation).size) do
      xhr :get, :destroy, :id => @conversation.to_param, :base_mail => @trashed_mail.to_param
    end
    assert_response :success
    assert_template 'conversations/destroy'
  end  
